...
/** @defgroup DriverIcm207xx Icm207xx driver
 *  @brief    Low-level driver for Icm207xx devices
 *  @ingroup  Drivers
 *  @{
 */

#ifndef _INV_ICM207XX_H_
#define _INV_ICM207XX_H_

#include "Invn/InvBool.h"
#include "Invn/InvError.h"

#include "Invn/Devices/Drivers/Icm207xx/Icm207xxCtrl.h"
#include "Invn/Devices/Drivers/Icm207xx/Icm207xxSelfTest.h"
#include "Invn/Devices/Drivers/Icm207xx/Icm207xxSerif.h"
#include "Invn/Devices/Drivers/Icm207xx/Icm207xxSetup.h"
#include "Invn/Devices/Drivers/Icm207xx/Icm207xxTransport.h"
#include "Invn/Devices/Drivers/Icm207xx/Icm207xxAuxTransport.h"
#include "Invn/Devices/Drivers/Icm207xx/Icm207xxAuxCompassAkm.h"
...
/** @brief Icm207xx driver states definition
 */
typedef struct inv_icm207xx {
	struct inv_icm207xx_serif serif;
	struct inv_icm207xx_serif serif_ois;
	inv_bool_t use_serif_ois; /* set to 1 is ois serif has been set */

	/** @brief icm207xx internal state structure*/
	struct inv_icm207xx_states {
		uint8_t  wake_state;
		uint8_t  pwr_mgmt_1;
		uint8_t  pwr_mgmt_2;
		uint8_t  lp_config;
	...
	uint16_t requested_odr[INV_ICM207XX_SENSOR_MAX];
	uint8_t  dropped_data[INV_ICM207XX_SENSOR_MAX];
	...
	struct inv_icm207xx_secondary_states {
		/** @brief secondary register mapping*/
		struct inv_icm207xx_secondary_reg {
			uint16_t addr;
			uint16_t reg;
			uint16_t ctrl;
			uint16_t d0;
 ...
 */
extern void inv_icm207xx_sleep(int ms);
 ...
 */
extern uint64_t inv_icm207xx_get_dataready_interrupt_time_us(void);
 ...
 */
static inline void inv_icm207xx_reset_states(struct inv_icm207xx * s,
		const struct inv_icm207xx_serif * serif)
{
	memset(s, 0, sizeof(*s));
	s->serif = *serif;
}
 ...
 *  Must be called after inv_icm207xx_reset_states()
 *  @param[in] s handle to driver states structure
 *  @param[in] serif handle to SERIF object for underlying register access to OIS
 */
static inline void inv_icm207xx_reset_states_serif_ois(struct inv_icm207xx * s,
        const struct inv_icm207xx_serif * serif_ois)
{
	if(serif_ois) {
