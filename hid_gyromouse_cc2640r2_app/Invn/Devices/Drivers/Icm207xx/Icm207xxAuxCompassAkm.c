...
#include "Icm207xxAuxCompassAkm.h"
#include "Icm207xxAuxTransport.h"
#include "Icm207xxTransport.h"
#include "Icm207xxSerif.h"
...
void inv_icm207xx_register_aux_compass(struct inv_icm207xx * s,
		enum inv_icm207xx_compass_id compass_id, uint8_t compass_i2c_addr)
{
	/* only for 20690 for now */

#if (MEMS_CHIP == HW_ICM20690)
	switch(compass_id) {
	case INV_ICM207XX_COMPASS_ID_AK09911:
		s->secondary_state.compass_id       = HW_AK09911;
		s->secondary_state.compass_i2c_addr = compass_i2c_addr;
		break;
	case INV_ICM207XX_COMPASS_ID_AK09912:
...
int inv_icm207xx_is_compass_registered(struct inv_icm207xx * s)
{
 ...
 */
int inv_icm207xx_setup_compass_akm(struct inv_icm207xx * s)
{
	int result;
	...
	result = inv_icm207xx_execute_read_secondary(s, COMPASS_I2C_SLV_READ,
	...
	result = inv_icm207xx_read_secondary(s, COMPASS_I2C_SLV_READ,
			s->secondary_state.compass_i2c_addr, sens, THREE_AXES);
	if (result)
		return result;

	result = inv_icm207xx_execute_write_secondary(s, COMPASS_I2C_SLV_WRITE,
			s->secondary_state.compass_i2c_addr, s->secondary_state.mode_reg_addr, cmd);
	if (result)
		return result;
	
	result = inv_icm207xx_mems_read_reg(s, MPUREG_EXT_SLV_SENS_DATA_00,
	...
	return inv_icm207xx_suspend_akm(s);
}

int inv_icm207xx_check_akm_self_test(struct inv_icm207xx * s)
{
	int result;
	...
	/* back up registers */
	/* SLV0_CTRL */
	result = inv_icm207xx_mems_read_reg(s, MPUREG_I2C_SLV0_CTRL, 1, &slv_ctrl[0]);
	if (result)
		return result;
	d = 0;
	result = inv_icm207xx_mems_write_reg(s, MPUREG_I2C_SLV0_CTRL, 1, &d);
	if (result)
		return result;
	/* SLV1_CTRL */
	result = inv_icm207xx_mems_read_reg(s, MPUREG_I2C_SLV1_CTRL, 1, &slv_ctrl[1]);
	if (result)
		return result;
	d = 0;
	result = inv_icm207xx_mems_write_reg(s, MPUREG_I2C_SLV1_CTRL, 1, &d);
	if (result)
		return result;
	...
	result = inv_icm207xx_write_secondary(s, 0, addr, mode, DATA_AKM_MODE_PD);
	if (result)
		goto AKM_fail;
	...
	if ((HW_AK09911 != s->secondary_state.compass_id) &&
		...
		result = inv_icm207xx_write_secondary(s, 0, addr, REG_AKM_ST_CTRL, DATA_AKM_SELF_TEST);
		if (result)
			goto AKM_fail;
	}
	...
	/* set self test mode */
	if (HW_AK09911 == s->secondary_state.compass_id)
		result = inv_icm207xx_write_secondary(s, 0, addr, mode, DATA_AK09911_MODE_ST);
	else if (HW_AK09912 == s->secondary_state.compass_id)
		result = inv_icm207xx_write_secondary(s, 0, addr, mode, DATA_AK09912_MODE_ST);
	else
		result = inv_icm207xx_write_secondary(s, 0, addr, mode,	DATA_AKM_MODE_ST);
	...
	while (counter > 0) {
		inv_icm207xx_sleep(15);

		if (HW_AK09911 == s->secondary_state.compass_id)
			result = inv_icm207xx_execute_read_secondary(s, 0, addr, REG_AK09911_STATUS1, 1, data);
		else if (HW_AK09912 == s->secondary_state.compass_id)
			result = inv_icm207xx_execute_read_secondary(s, 0, addr, REG_AK09912_STATUS1, 1, data);
		else
			result = inv_icm207xx_execute_read_secondary(s, 0, addr, REG_AKM_STATUS, 1, data);
		if (result)
			goto AKM_fail;
	...
	if (HW_AK09911 == s->secondary_state.compass_id) {
		result = inv_icm207xx_execute_read_secondary(s, 0, addr,REG_AK09911_MEASURE_DATA, BYTES_PER_SENSOR, data);
	} else if (HW_AK09912 == s->secondary_state.compass_id) {
		result = inv_icm207xx_execute_read_secondary(s, 0, addr, REG_AK09912_MEASURE_DATA, BYTES_PER_SENSOR, data);
	} else {
		result = inv_icm207xx_execute_read_secondary(s, 0, addr, REG_AKM_MEASURE_DATA, BYTES_PER_SENSOR, data);
	}
	if (result)
		goto AKM_fail;
	...
	if (s->secondary_state.compass_sens[0] &&
		...
		if (HW_AK8963 == s->secondary_state.compass_id) {
			result = inv_icm207xx_execute_read_secondary(s, 0, addr, REG_AKM8963_CNTL1, 1, &cntl);
			if (result)
				goto AKM_fail;
...
AKM_fail:
	...
	if ((HW_AK09911 != s->secondary_state.compass_id) &&
		(HW_AK09912 != s->secondary_state.compass_id)) {
		result |= inv_icm207xx_write_secondary(s, 0, addr, REG_AKM_ST_CTRL, 0);
	}
	/*set to power down mode */
	result |= inv_icm207xx_write_secondary(s, 0, addr, mode, DATA_AKM_MODE_PD);
 ...
 */
int inv_icm207xx_write_akm_scale(struct inv_icm207xx * s, int data)
{
	unsigned char d, en;
	int result;
	...
	result = inv_icm207xx_mems_write_reg(s, MPUREG_I2C_SLV1_DO, 1, &d);
	if (result)
		return result;
 ...
 */
int inv_icm207xx_read_akm_scale(struct inv_icm207xx * s, int *scale)
{
	if (HW_AK8975 == s->secondary_state.compass_id)
...
int inv_icm207xx_suspend_akm(struct inv_icm207xx * s)
{
	int result;
	...
	result = inv_icm207xx_secondary_disable_i2c(s);

	/* slave 0 is disabled */
	result |= inv_icm207xx_secondary_stop_channel(s, COMPASS_I2C_SLV_READ);
	/* slave 1 is disabled */
	result |= inv_icm207xx_secondary_stop_channel(s, COMPASS_I2C_SLV_WRITE);
	if (result)
		return result;

	/* Enable AUX I2C again if there are other devices in use */
	// TODO: Check other devices on AUX I2C
	//result |= inv_icm207xx_secondary_enable_i2c(s);
...
int inv_icm207xx_resume_akm(struct inv_icm207xx * s)
{
	int result;
	...
	result = inv_icm207xx_read_secondary(s, COMPASS_I2C_SLV_READ, s->secondary_state.compass_i2c_addr, reg_addr, bytes);
	if (result)
		return result;
	...
	result = inv_icm207xx_write_secondary(s, COMPASS_I2C_SLV_WRITE, s->secondary_state.compass_i2c_addr, s->secondary_state.mode_reg_addr, lDataToWrite);
	if (result)
		return result;
...
char inv_icm207xx_compass_getstate(struct inv_icm207xx * s)
{
	return s->secondary_state.secondary_resume_state;
}

int inv_icm207xx_get_compass_data(struct inv_icm207xx * s,
...
int inv_icm207xx_get_compass_bytes(struct inv_icm207xx * s)
{
	int bytes = 0;
