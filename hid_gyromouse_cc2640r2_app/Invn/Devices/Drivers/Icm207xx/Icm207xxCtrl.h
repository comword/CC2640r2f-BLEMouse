...
#ifndef _INV_ICM207XX_CTRL_H_
#define _INV_ICM207XX_CTRL_H_
 ...
 */
enum inv_icm207xx_sensor {
	INV_ICM207XX_SENSOR_ACCEL,
	INV_ICM207XX_SENSOR_GYRO,
	INV_ICM207XX_SENSOR_COMPASS,
	INV_ICM207XX_SENSOR_FSYNC_EVENT,
	INV_ICM207XX_SENSOR_OIS,
	INV_ICM207XX_SENSOR_TEMPERATURE,
	INV_ICM207XX_SENSOR_MAX
};
 ...
 */
int INV_EXPORT inv_icm207xx_enable_mems(struct inv_icm207xx * s, int bit_mask, uint16_t smplrt_divider);
 ...
 */
int INV_EXPORT inv_icm207xx_set_sensor_period(struct inv_icm207xx * s, enum inv_icm207xx_sensor sensor, uint16_t delayInMs);

/** @brief Knows if the sensor is enabled or disabled
 *  @param[in] sensor  the sensor is enabled or not
 *  @return 1 if active, 0 otherwise
 */
int INV_EXPORT inv_icm207xx_is_sensor_enabled(struct inv_icm207xx * s, enum inv_icm207xx_sensor sensor);
 ...
 */
int INV_EXPORT inv_icm207xx_enable_sensor(struct inv_icm207xx * s, enum inv_icm207xx_sensor sensor, uint8_t enable);
 ...
 *  @return 0 on success, negative value on error.
 */
int INV_EXPORT inv_icm207xx_configure_accel_wom(struct inv_icm207xx * s, uint8_t wom_threshold);
 ...
 *          < 0  : error
 */
int INV_EXPORT inv_icm207xx_poll_sensor_data_reg(struct inv_icm207xx * s, 
		int16_t acc_data[3], int16_t *temp_data, int16_t gyro_data[3]);

/** @brief Check and retrieve for new data
 */
struct inv_icm207xx_fifo_states {
 ...
 *  @param[in] states     place holder to FIFO states
 *  @param[in] int_status int status register value (used to check for overflow)
 *  @return    0 for success, 1 for overflow detected, negative value for other errors
 */
int INV_EXPORT inv_icm207xx_poll_fifo_data_setup(struct inv_icm207xx * s,
 ...
 *          < 0  : error
  */
int INV_EXPORT inv_icm207xx_poll_fifo_data(struct inv_icm207xx * s, struct inv_icm207xx_fifo_states * states, 
 ...
 */
int INV_EXPORT inv_icm207xx_has_data_ready(struct inv_icm207xx * s);

/** @brief Read interrupt status and return its value
 *  @param[out] int_status INT status register value
 *  @return 0 on success, negative value on error
 */
int INV_EXPORT inv_icm207xx_get_int_status(struct inv_icm207xx * s, uint8_t * int_status);
 ...
 */
int INV_EXPORT inv_icm207xx_check_drdy(struct inv_icm207xx * s, uint8_t int_status);
 ...
 */
int INV_EXPORT inv_icm207xx_check_wom_status(struct inv_icm207xx * s, uint8_t int_status);

/** @brief Disable FIFO
 * @return 0 on success, negative value on error.
 */
int INV_EXPORT inv_icm207xx_disable_fifo(struct inv_icm207xx * s);

/** @brief    Enable FIFO
 * @param[in] bit_mask 	A mask of sensor to push to FIFO.
 * @return    0 on success, negative value on error.
 */
int INV_EXPORT inv_icm207xx_enable_fifo(struct inv_icm207xx * s, int bit_mask);

/** @brief  Reset FIFO
 *  @return 0 on success, negative value on error.
 */
int INV_EXPORT inv_icm207xx_reset_fifo(struct inv_icm207xx * s);

/** @brief  Initalize AUX I2C and Compass. Only supported in ICM20690.
 *  @return 0 on success, negative value on error.
 */
int INV_EXPORT inv_icm207xx_set_slave_compass_id(struct inv_icm207xx * s);

/** @brief  Poll OIS gyro data. Only supported in ICM20690.
 *  @return 0x10 if OIS data reported, 0 otherwise, negative value on error.
 */
int INV_EXPORT inv_icm207xx_poll_ois_gyro_data(struct inv_icm207xx * s, int16_t gyro_data[3]);

/** @brief  Poll EIS flag and delay counter. Only supported in ICM20690.
 ...
 */
int INV_EXPORT inv_icm207xx_poll_delay_count(struct inv_icm207xx * s, int16_t * delay_count);

/** @brief  test if all sensors are off
 *  @return true if all sensors are off, false otherwise
 */
int INV_EXPORT inv_icm207xx_all_sensors_off(struct inv_icm207xx * s);

#ifdef __cplusplus
}
#endif

#endif /* _INV_ICM207XX_CTRL_H_ */
