...
#include "Icm207xxCtrl.h"
#include "Icm207xxDefs.h"
#include "Icm207xx.h"
...
/* Convert public sensor type for Icm207xx to internal sensor id (Android)
 */
static uint8_t sensor_type_2_android_sensor(enum inv_icm207xx_sensor sensor)
{
	switch(sensor) {
	case INV_ICM207XX_SENSOR_ACCEL:       return ANDROID_SENSOR_ACCELEROMETER;
	case INV_ICM207XX_SENSOR_GYRO:        return ANDROID_SENSOR_GYROSCOPE;
	case INV_ICM207XX_SENSOR_FSYNC_EVENT: return ANDROID_SENSOR_FSYNC_EVENT;
	case INV_ICM207XX_SENSOR_OIS:         return ANDROID_SENSOR_OIS;
	case INV_ICM207XX_SENSOR_TEMPERATURE: return ANDROID_SENSOR_TEMPERATURE;
	case INV_ICM207XX_SENSOR_COMPASS:     return ANDROID_SENSOR_MAGNETOMETER;
	default:                                    return ANDROID_SENSOR_NUM_MAX;
	}
}

static uint32_t is_android_sensor_enabled(struct inv_icm207xx * s, uint8_t androidSensor)
{
	return (s->android_sensor_mask & (1L << (androidSensor & 0x1F)));
}

static int apply_accel_fsr(struct inv_icm207xx * s)
{
	int result = 0;
	uint8_t accel_config_1_reg;

	// Set FSR
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_ACCEL_CONFIG, 1, &accel_config_1_reg);
	accel_config_1_reg &= ~BITS_ACCEL_FS_SEL;//0xE7;
	accel_config_1_reg |= (s->base_state.accel_fullscale << BIT_POS_ACCEL_FS_SEL);
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_CONFIG, 1, &accel_config_1_reg);

	return result;
}

static int apply_accel_bw(struct inv_icm207xx * s)
{
	int result = 0;
	...
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_ACCEL_CONFIG_2, 1, &accel_config_2_reg);
	accel_config_2_reg &= ~BITS_DEC2_CFG;
	accel_config_2_reg &= ~BITS_A_DLPF_CFG;

	// Set DEC2 and A_DLPF_CFG
	if(inv_icm207xx_get_chip_power_state(s) & CHIP_LP_ENABLE) // accel LP mode
	{
	...
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_CONFIG_2, 1, &accel_config_2_reg);

	return result;
}

static int apply_accel_bias(struct inv_icm207xx * s)
{
	int i, axis_sign;
	int rc = -1;
	...
	if(!s->accel_reg_bias_computed) {
		...
		rc = inv_icm207xx_mems_read_reg(s, MPUREG_XA_OFFS_H, 2, d);
		accel_reg_otp[0] = ((uint16_t)d[0] << 8) | d[1];
		rc = inv_icm207xx_mems_read_reg(s, MPUREG_YA_OFFS_H, 2, d);
		accel_reg_otp[1] = ((uint16_t)d[0] << 8) | d[1];
		rc = inv_icm207xx_mems_read_reg(s, MPUREG_ZA_OFFS_H, 2, d);
	...
	rc = inv_icm207xx_mems_write_reg(s, MPUREG_XA_OFFS_H, 2, d);

	d[0] = (sensor_bias[1] >> 8) & 0xff;
	d[1] = sensor_bias[1] & 0xff;
	rc |= inv_icm207xx_mems_write_reg(s, MPUREG_YA_OFFS_H, 2, d);

	d[0] = (sensor_bias[2] >> 8) & 0xff;
	d[1] = sensor_bias[2] & 0xff;
	rc |= inv_icm207xx_mems_write_reg(s, MPUREG_ZA_OFFS_H, 2, d);

	return rc;
}

static int apply_gyr_fsr_bw(struct inv_icm207xx * s)
{
	int result = 0;
	uint8_t data;
	...
	// Set FSR
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_GYRO_CONFIG, 1, &gyro_config_1_reg);
	gyro_config_1_reg &= ~BITS_GYRO_FS_SEL;
	gyro_config_1_reg |= (inv_icm207xx_get_gyro_fullscale(s)<< BIT_POS_GYRO_FS_SEL);
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_GYRO_CONFIG, 1, &gyro_config_1_reg);
	...
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_LP_CONFIG, 1, &data);
	data &= ~BITS_G_AVGCFG;
	data |= (g_avgcfg<<BIT_POS_G_AVGCFG);
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_LP_CONFIG, 1, &data);

	// Set DLPF_CFG
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_CONFIG, 1, &data);
	data &= ~BITS_DLPF_CFG; // Clear bits[2:0]
	data |= inv_icm207xx_get_gyro_bandwidth(s);
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_CONFIG, 1, &data);

	return result;
}

static int apply_gyr_bias(struct inv_icm207xx * s)
{
	int i;
	int rc = -1;
	int sensor_bias[3];
	...
	rc = inv_icm207xx_mems_write_reg(s, MPUREG_XG_OFFS_USRH, 2, data);

	data[0] = (sensor_bias[1] >> 8) & 0xff;
	data[1] = sensor_bias[1] & 0xff;
	rc |= inv_icm207xx_mems_write_reg(s, MPUREG_YG_OFFS_USRH, 2, data);

	data[0] = (sensor_bias[2] >> 8) & 0xff;
	data[1] = sensor_bias[2] & 0xff;
	rc |= inv_icm207xx_mems_write_reg(s, MPUREG_ZG_OFFS_USRH, 2, data);

	return rc;
}

static int enable_interrupt(struct inv_icm207xx * s, int bit_mask)
{
	int result = 0;
	uint8_t data;

	result |= inv_icm207xx_mems_read_reg(s, MPUREG_INT_ENABLE, 1, &data);
	...
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_INT_ENABLE, 1, &data);

	return result;
}

int inv_icm207xx_has_data_ready(struct inv_icm207xx * s)
{
	uint8_t int_status;

	const int rc = inv_icm207xx_read_reg_one(s, REG_INT_STATUS, &int_status);
...
int inv_icm207xx_get_int_status(struct inv_icm207xx * s, uint8_t * int_status)
{
	return inv_icm207xx_read_reg_one(s, REG_INT_STATUS, int_status);
}

int inv_icm207xx_check_drdy(struct inv_icm207xx * s, uint8_t int_status)
{
	(void)s;
...
int inv_icm207xx_check_wom_status(struct inv_icm207xx * s, uint8_t int_status)
{
	int wom_data = 0;
...
int inv_icm207xx_poll_sensor_data_reg(struct inv_icm207xx * s, int16_t acc_data[3], int16_t * temp_data, int16_t gyro_data[3])
{
	int rc;
	uint8_t data[14];
	...
	const int acc_on = inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_ACCEL);
	const int gyro_on = inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_GYRO);
	const int temp_on = inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_TEMPERATURE);
	...
	if((rc = inv_icm207xx_read_reg(s, reg, &data[offset], len)) != 0)
		return rc;
	...
	if(acc_on) {
		...
		rc |= (1 << INV_ICM207XX_SENSOR_ACCEL);
	}
	...
	if(temp_on) {
		...
		*temp_data = (data[0+6] << 8) | data[1+6];
		...
		rc |= (1 << INV_ICM207XX_SENSOR_TEMPERATURE);
	}
	...
	if(gyro_on) {
		...
		rc |= (1 << INV_ICM207XX_SENSOR_GYRO);
	}

	return rc;
}

int inv_icm207xx_poll_fifo_data_setup(struct inv_icm207xx * s,
		struct inv_icm207xx_fifo_states * states, uint8_t int_status)
{
#if ((MEMS_CHIP == HW_ICM20602)||(MEMS_CHIP == HW_ICM20690)||(MEMS_CHIP == HW_ICM20603)||(MEMS_CHIP == HW_ICM207XX)||(MEMS_CHIP == HW_ICM20609))
	int rc;
	uint8_t data[2];
	...
	if(inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_ACCEL)) {
		states->sensor_on_mask |= (1 << INV_ICM207XX_SENSOR_ACCEL);
		states->packet_size += ACCEL_DATA_SIZE;
	}

	if(inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_GYRO)) {
		states->sensor_on_mask |= (1 << INV_ICM207XX_SENSOR_GYRO);
		states->packet_size += GYRO_DATA_SIZE;
	}
	
#if ((MEMS_CHIP == HW_ICM20602)||(MEMS_CHIP == HW_ICM20603))
	if(inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_ACCEL) ||
	   inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_GYRO) ) {
		states->packet_size += TEMP_DATA_SIZE;
	}
	if(inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_TEMPERATURE)) {
		states->sensor_on_mask |= (1 << INV_ICM207XX_SENSOR_TEMPERATURE);
	}
#endif

#if ((MEMS_CHIP == HW_ICM20690)||(MEMS_CHIP == HW_ICM207XX)||(MEMS_CHIP == HW_ICM20609))
	if(inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_TEMPERATURE)) {
		states->sensor_on_mask |= (1 << INV_ICM207XX_SENSOR_TEMPERATURE);
		states->packet_size += TEMP_DATA_SIZE;
	}
	...
	if(inv_icm207xx_is_sensor_enabled(s, INV_ICM207XX_SENSOR_COMPASS)) {
		states->sensor_on_mask |= (1 << INV_ICM207XX_SENSOR_COMPASS);
		states->packet_size += inv_icm207xx_get_compass_bytes(s);
	}
#endif
	...
	if(int_status & BIT_FIFO_OVERFLOW) {
		...
		for(i = 0; sensor_on_mask != 0 ; ++i) {
			if(sensor_on_mask & 1) {
				inv_icm207xx_enable_sensor(s, (enum inv_icm207xx_sensor)i, 0);
				inv_icm207xx_enable_sensor(s, (enum inv_icm207xx_sensor)i, 1);
			}
			sensor_on_mask >>= 1;
		}
	...
	if((rc = inv_icm207xx_read_reg(s, MPUREG_FIFO_COUNTH, data, 2)) != 0)
		return rc;
...
int inv_icm207xx_poll_fifo_data(struct inv_icm207xx * s,
		struct inv_icm207xx_fifo_states * states, int16_t acc_data[3],
		int16_t * temp_data, int16_t gyro_data[3], int16_t compass_data[3])
{
	...
	if(states->packet_count_i < states->packet_count) {
		...
		if((rc = inv_icm207xx_read_reg(s, MPUREG_FIFO_R_W, data, states->packet_size)) != 0) {
			/* sensor data is in FIFO according to FIFO_COUNT but failed to read FIFO,
				   reset FIFO and try next chance */
			inv_icm207xx_reset_fifo(s);
			return rc;
		}

		if(states->sensor_on_mask & (1 << INV_ICM207XX_SENSOR_ACCEL)) {
			if(!s->dropped_data[INV_ICM207XX_SENSOR_ACCEL]) {
				...
				s->dropped_data[INV_ICM207XX_SENSOR_ACCEL]--;
		...
		if(states->sensor_on_mask) {
			...
			*temp_data = (data[0+idx] << 8) | data[1+idx];
			...
			if(states->sensor_on_mask & (1 << INV_ICM207XX_SENSOR_TEMPERATURE)) {
				if(!s->dropped_data[INV_ICM207XX_SENSOR_TEMPERATURE])
					rc |= (1 << INV_ICM207XX_SENSOR_TEMPERATURE);
				else
					s->dropped_data[INV_ICM207XX_SENSOR_TEMPERATURE]--;
			}
			idx += TEMP_DATA_SIZE;
		}
#endif
		...
		if(states->sensor_on_mask & (1 << INV_ICM207XX_SENSOR_GYRO)) {
			if(!s->dropped_data[INV_ICM207XX_SENSOR_GYRO]) {
				if(!repeated_mems_data) {
				...
				s->dropped_data[INV_ICM207XX_SENSOR_GYRO]--;

			idx += GYRO_DATA_SIZE;
		}
		
#if (MEMS_CHIP == HW_ICM20690)
		if(states->sensor_on_mask & (1 << INV_ICM207XX_SENSOR_COMPASS)) {
			if(!repeated_compass_data) {
				p = &data[idx];
				inv_icm207xx_get_compass_data(s, p, d);
				compass_data[0] = (d[0] << 8) | d[1];
				compass_data[1] = (d[2] << 8) | d[3];
				compass_data[2] = (d[4] << 8) | d[5];

				rc |= (1 << INV_ICM207XX_SENSOR_COMPASS);
			}
			repeated_compass_data = (repeated_compass_data+1) % (s->secondary_state.i2c_mst_dly+1);
			idx += inv_icm207xx_get_compass_bytes(s);
		}
#endif
	...
	if(states->packet_count_i < states->packet_count) {
		states->packet_count_i += 1;
		return inv_icm207xx_poll_sensor_data_reg(s, acc_data, temp_data, gyro_data);
	}
	else {
		return 0;
	}
...
int inv_icm207xx_enable_mems(struct inv_icm207xx * s, int bit_mask, uint16_t smplrt_divider)
{
	int result = 0;
	...
	uint8_t data = 0;
	uint8_t fs_sel = 0;
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_USER_CTRL_NEW, 1, &data);

	if(bit_mask & OIS_AVAILABLE) {
		// Set FSR
		result |= inv_icm207xx_mems_read_reg(s, MPUREG_ACCEL_CONFIG, 1, &fs_sel);
		fs_sel &= ~BITS_ACCEL_FS_SEL_OIS;
		fs_sel |= (s->base_state.accel_ois_fullscale);
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_CONFIG, 1, &fs_sel);

		result |= inv_icm207xx_mems_read_reg(s, MPUREG_SIGNAL_PATH_RESET, 1, &fs_sel);
		fs_sel &= ~BITS_GYRO_FS_SEL_OIS;
		fs_sel |= (s->base_state.gyro_ois_fullscale << 5);
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_SIGNAL_PATH_RESET, 1, &fs_sel);
	...
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_USER_CTRL_NEW, 1, &data);
#endif

#if (MEMS_CHIP == HW_ICM207XX)
	if(smplrt_divider == 1)
	...
	case 0 :
		...
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_PWR_MGMT_1, 1, &s->base_state.pwr_mgmt_1);

		s->base_state.pwr_mgmt_2 |= BIT_PWR_ALL_OFF;
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_PWR_MGMT_2, 1, &s->base_state.pwr_mgmt_2);
	...
	case ACCEL_AVAILABLE : // to accel Low Power
		...
		switch(s->power_state) 
		{
			case PowerStateSleepState:
				...
				inv_icm207xx_sleep(20); // accel startup time
				break;
	...
	case GYRO_AVAILABLE : // to gyro Low Power
		...
		switch(s->power_state) 
		{
			case PowerStateSleepState:
...
#endif
				result |= inv_icm207xx_mems_write_reg(s, MPUREG_LP_CONFIG, 1, &s->base_state.lp_config);
				s->base_state.pwr_mgmt_2 |= BIT_PWR_ACCEL_STBY;
				s->base_state.pwr_mgmt_2 &= ~BIT_PWR_GYRO_STBY;
				result |= inv_icm207xx_mems_write_reg(s, MPUREG_PWR_MGMT_2, 1, &s->base_state.pwr_mgmt_2);
				s->base_state.pwr_mgmt_1 &= ~BIT_SLEEP;
				...
				inv_icm207xx_sleep(50); // gyro startup time
				break;
			case PowerStateAccLPState:
				//Procedure 19
#if (MEMS_CHIP == HW_ICM20603)
	...
	if (bit_mask & COMPASS_AVAILABLE) {
		...
		s->base_state.wake_state |= CHIP_AWAKE;
		s->base_state.wake_state &= ~CHIP_LP_ENABLE;
		result |= inv_icm207xx_set_accel_bandwidth(s, MPU_ABW_218);
		apply_accel_bw(s);
		inv_icm207xx_sleep(20); // accel startup time
		...
		result |= inv_icm207xx_resume_akm(s);
	} else {
		if (s->power_state == PowerStateAccLPState) {
			...
			result |= inv_icm207xx_mems_write_reg(s, MPUREG_PWR_MGMT_1, 1, &s->base_state.pwr_mgmt_1);
			s->base_state.wake_state |= CHIP_LP_ENABLE;
			result |= inv_icm207xx_set_accel_bandwidth(s, MPU_ABW_420);
			apply_accel_bw(s);
		}
		result |= inv_icm207xx_suspend_akm(s);
	}
#endif
...
typedef	struct {
	...
	enum inv_icm207xx_sensor InvSensor;
} MinDelayGenElementT;

/******************/
//	accel
static const MinDelayGenElementT MinDelayGenAccelList[]	= {
	{ ANDROID_SENSOR_ACCELEROMETER, INV_ICM207XX_SENSOR_ACCEL }
};

//	gyro
static const MinDelayGenElementT MinDelayGenGyroList[] = {
	{ ANDROID_SENSOR_GYROSCOPE, INV_ICM207XX_SENSOR_GYRO }
};

//	compass
static const MinDelayGenElementT MinDelayGenCompassList[] = {
	{ ANDROID_SENSOR_MAGNETOMETER, INV_ICM207XX_SENSOR_COMPASS }
};
...
static MinDelayT MinDelayGenActual(struct inv_icm207xx * s, const MinDelayGenElementT * Element, u32 ElementQuan)
{
...
/******************/
static u16 SampleRateDividerGet(struct inv_icm207xx * s, MinDelayT MinDelay)
{
	//u16	Delay = INV_MIN(ODR_MIN_DELAY, MinDelay);
	return MinDelay * inv_icm207xx_get_chip_base_sample_rate(s)/1000L; // a divider from BASE_SAMPLE_RATE Hz.
}

/******************/
static int set_hw_smplrt_dmp_odrs(struct inv_icm207xx * s, uint16_t * resulting_divider)
{
	int result = 0;
	uint8_t data = 0;
	...
	{
		result |= inv_icm207xx_mems_read_reg(s, MPUREG_ACCEL_CONFIG_2, 1, &data);
		data &= ~BITS_ACCEL_FCHOICE_B; // Clear ACCEL_FCHOICE_B
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_CONFIG_2, 1, &data);
	}

	// Set fchoice_b to 0, dlpf_cfg to 1
	if(s->android_sensor_mask & INV_NEEDS_GYRO_MASK)
	{
		result |= inv_icm207xx_mems_read_reg(s, MPUREG_GYRO_CONFIG, 1, &data);
		data &= ~BITS_FCHOICE_B; // Clear FCHOICE_B
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_GYRO_CONFIG, 1, &data);
			...
			(s->android_sensor_mask & INV_NEEDS_COMPASS_MASK))
	{
		...
		if (minDly != 0xFFFF)
		{
			...
			result |= inv_icm207xx_set_divider(s, (uint8_t)(hw_smplrt_divider - 1));
			if(s->android_sensor_mask & INV_NEEDS_COMPASS_MASK)
				result |= inv_icm207xx_set_i2c_mst_dly(s, minDly, minDly_compass);
		}
	}
	...
	if (s->android_sensor_mask & INV_NEEDS_OIS_MASK) {
		...
		result |= inv_icm207xx_mems_read_reg(s, MPUREG_SIGNAL_PATH_RESET, 1, &data);
		data &= ~BITS_FCHOICE_OIS_B;
		data |= (1 << BIT_POS_FCHOICE_OIS_B);
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_SIGNAL_PATH_RESET, 1, &data);

		// Set accel_fchoice_ois_b to 11
		result |= inv_icm207xx_mems_read_reg(s, MPUREG_ACCEL_INTEL_CTRL, 1, &data);
		data &= ~BITS_ACCEL_FCHOICE_OIS_B;
		data |= (3 << BIT_POS_ACCEL_FCHOICE_OIS_B);
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_INTEL_CTRL, 1, &data);
	}
#endif
...
int inv_icm207xx_set_sensor_period(struct inv_icm207xx * s, enum inv_icm207xx_sensor sensor, uint16_t delayInMs)
{
	int result = 0;
	...
	case ANDROID_SENSOR_ACCELEROMETER:
		s->requested_odr[INV_ICM207XX_SENSOR_ACCEL] = delayInMs;
		break;
	...
	result |= inv_icm207xx_disable_fifo(s);
	...
	result |= inv_icm207xx_enable_mems(s, (int)data_rdy_status, resulting_divider);

	// need to reset and restart FIFO with new configurations
	result |= inv_icm207xx_enable_fifo(s, (int)data_rdy_status);

	return result;
}

int inv_icm207xx_is_sensor_enabled(struct inv_icm207xx * s, enum inv_icm207xx_sensor sensor)
{
	const uint8_t androidSensor = sensor_type_2_android_sensor(sensor);
...
static void regenerate_sensor_control(struct inv_icm207xx * s,
 ...
 */
static void convert_android_sensor_to_control(struct inv_icm207xx * s,
...
int inv_icm207xx_enable_sensor(struct inv_icm207xx * s, enum inv_icm207xx_sensor sensor, uint8_t enable)
{
	int result = 0;
	uint16_t data_rdy_status = 0;
	uint16_t resulting_divider = 0;
	...
	if (s->android_sensor_mask & INV_NEEDS_FSYNC_MASK) {
		inv_icm207xx_set_fsync_bit_location(s, enable);
	}
	...
	if(s->dropped_data[INV_ICM207XX_SENSOR_ACCEL] && s->dropped_data[INV_ICM207XX_SENSOR_GYRO]) {
		s->dropped_data[INV_ICM207XX_SENSOR_ACCEL] = 2;
		s->dropped_data[INV_ICM207XX_SENSOR_GYRO] = 2;
	}

	return result;
}

int inv_icm207xx_configure_accel_wom(struct inv_icm207xx * s, uint8_t wom_threshold)
{
	int result = 0;
	uint8_t data;
	...
	if(wom_threshold) {
		// Set threshold
		// wom_threshold = 0x0F;
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_WOM_THR,   1, &wom_threshold);
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_WOM_X_THR, 1, &wom_threshold);
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_WOM_Y_THR, 1, &wom_threshold);
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACCEL_WOM_Z_THR, 1, &wom_threshold);
		...
		result |= inv_icm207xx_mems_read_reg(s, MPUREG_PWR_MGMT_1, 1, &data);
		data &= ~BIT_SLEEP; // Clear sleep bit
		data |= BIT_CYCLE; // Enable accel cycling mode.
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_PWR_MGMT_1, 1, &data);

		// Enable accel
		result |= inv_icm207xx_mems_read_reg(s, MPUREG_PWR_MGMT_2, 1, &data);
		data &= ~BIT_PWR_ACCEL_STBY; // Enable accel
		data |= BIT_PWR_GYRO_STBY; // Disable gyro.
		result |= inv_icm207xx_mems_write_reg(s, MPUREG_PWR_MGMT_2, 1, &data);
...
int inv_icm207xx_disable_fifo(struct inv_icm207xx * s)
{
#if ((MEMS_CHIP == HW_ICM20602)||(MEMS_CHIP == HW_ICM20690)||(MEMS_CHIP == HW_ICM20603)||(MEMS_CHIP == HW_ICM207XX)||(MEMS_CHIP == HW_ICM20609))
	int result = 0;
	uint8_t reg;
	...
	/* disable FIFO */
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_USER_CTRL, 1, &reg);
	reg &= ~BIT_FIFO_EN;
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_USER_CTRL, 1, &reg);

	/* reset FIFO_EN as well */
	reg = 0;
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_FIFO_EN, 1, &reg);
...
int inv_icm207xx_enable_fifo(struct inv_icm207xx * s, int bit_mask)
{
#if ((MEMS_CHIP == HW_ICM20602)||(MEMS_CHIP == HW_ICM20690)||(MEMS_CHIP == HW_ICM20603)||(MEMS_CHIP == HW_ICM207XX)||(MEMS_CHIP == HW_ICM20609))
	int result = 0;
	uint8_t reg;
	...
	if (bit_mask == 0) {
		inv_icm207xx_disable_fifo(s);
		return 0;
	}

	/* stop and reset FIFO */
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_USER_CTRL, 1, &user_ctrl);
	...
	reg = 0;
#if ((MEMS_CHIP == HW_ICM207XX) || (MEMS_CHIP == HW_ICM20609))
...
int inv_icm207xx_reset_fifo(struct inv_icm207xx * s)
{
#if ((MEMS_CHIP == HW_ICM20602)||(MEMS_CHIP == HW_ICM20690)||(MEMS_CHIP == HW_ICM20603)||(MEMS_CHIP == HW_ICM207XX)||(MEMS_CHIP == HW_ICM20609))
	int result = 0;
	uint8_t reg;
...
int inv_icm207xx_set_slave_compass_id(struct inv_icm207xx * s)
{
	int result = 0;

#if (MEMS_CHIP == HW_ICM20690)
	/* initialize aux i2c */
	inv_icm207xx_init_secondary(s);
	/* initialize akm compass */
	result |= inv_icm207xx_setup_compass_akm(s);
#else
	(void)s;
#endif
...
int inv_icm207xx_poll_ois_gyro_data(struct inv_icm207xx * s, int16_t gyro_data[3])
{
#if (MEMS_CHIP == HW_ICM20690)
	uint8_t data[6];
	int rc = 0;
...
int inv_icm207xx_poll_delay_count(struct inv_icm207xx * s, int16_t * delay_count)
{
#if (MEMS_CHIP == HW_ICM20690)
	uint8_t data[2];
	int rc = 0;
	if (inv_icm207xx_read_reg(s, MPUREG_TEMP_XOUT_L, data, 1) == 0) {
		if (data[0] & 0x1 /* keep the eis flag (bit 0) */) {

			rc |= inv_icm207xx_read_reg(s, MPUREG_ODR_DLY_CNT_HI, data, 2);
			*delay_count = (data[0] << 8) | data[1];

			rc |= (1 << INV_ICM207XX_SENSOR_FSYNC_EVENT);
		}
	}
...
int inv_icm207xx_all_sensors_off(struct inv_icm207xx * s)
{
