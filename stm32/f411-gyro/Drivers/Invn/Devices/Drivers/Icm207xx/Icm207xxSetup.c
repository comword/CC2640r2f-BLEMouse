...
static int serif_write(struct inv_icm207xx * s, uint16_t reg, uint32_t length, const uint8_t *data)
{
	return inv_icm207xx_write_reg(s, (uint8_t)reg, data, length);
}

static int serif_read(struct inv_icm207xx * s, uint16_t reg, uint32_t length, uint8_t *data)
{
	return inv_icm207xx_read_reg(s, (uint8_t)reg, data, length);
}

int inv_icm207xx_get_whoami(struct inv_icm207xx * s, uint8_t * whoami)
{
	return inv_icm207xx_read_reg_one(s, REG_WHO_AM_I, whoami);
}

int inv_icm207xx_get_chip_info(struct inv_icm207xx * s, uint8_t chip_info[3])
{
	int result = inv_icm207xx_read_reg_one(s, REG_WHO_AM_I, &chip_info[0]);

#if ((MEMS_CHIP == HW_ICM20602)||(MEMS_CHIP == HW_ICM20603))
	result |= inv_icm207xx_read_reg_one(s, MPUREG_MANUFACTURER_ID, &chip_info[1]);
	result |= inv_icm207xx_read_reg_one(s, MPUREG_CHIP_ID, &chip_info[2]);
#else
	chip_info[1] = 0;
	chip_info[2] = 0;
#endif
...
int inv_icm207xx_initialize(struct inv_icm207xx * s)
{
	int result = 0;
	uint8_t data = 0;
	unsigned i;
	...
	result |= inv_icm207xx_soft_reset(s);

	s->base_state.pwr_mgmt_1 = CLK_SEL;
	result |= serif_write(s, MPUREG_PWR_MGMT_1, 1, &s->base_state.pwr_mgmt_1);

#if ((MEMS_CHIP == HW_ICM6800)||(MEMS_CHIP == HW_ICM207XX)||(MEMS_CHIP == HW_ICM20609))
	if(s->serif.is_spi) {
		s->base_state.user_ctrl |= BIT_I2C_IF_DIS;
	}
	result |= inv_icm207xx_mems_write_reg(s, REG_USER_CTRL, 1, &s->base_state.user_ctrl);
#endif

	result |= serif_write(s, MPUREG_PWR_MGMT_2, 1, &s->base_state.pwr_mgmt_2);

	result |= inv_icm207xx_mems_read_reg(s, REG_WHO_AM_I, 1, &data);
	...
	inv_icm207xx_set_divider(s, FIFO_DIVIDER);       //Initial sampling rate BASE_SAMPLE_RATE/(9+1) = 100Hz for 1000Hz
	result |= inv_icm207xx_set_gyro_fullscale(s, MPU_FS_2000dps);
	result |= inv_icm207xx_set_accel_fullscale(s, MPU_FS_4G);
#if (MEMS_CHIP == HW_ICM20690)
	result |= inv_icm207xx_set_gyro_ois_fullscale(s, MPU_FS_250dps);
	result |= inv_icm207xx_set_accel_ois_fullscale(s, MPU_OIS_FS_4G);
#endif
	...
	result |= inv_icm207xx_set_gyro_bandwidth(s, MPU_GBW_176);
	...
	data = BITS_FIFO_SIZE; // 1kB FIFO for 20690, 4kB for 207xx and 512B for 20609
	...
	if(s->secondary_state.compass_id != 0) {
		inv_icm207xx_set_slave_compass_id(s);
	}
#endif

	// Enable Interrupts.
	data = BIT_DATA_RDY_INT_EN;
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_INT_ENABLE, 1, &data); // Enable DRDY Interrupt

	return result;
}

int inv_icm207xx_soft_reset(struct inv_icm207xx * s)
{
	int result = 0;
	int timeout = 1000; /* 1s */
	...
	inv_icm207xx_sleep(50); // wait for 50ms after soft reset
...
int inv_icm207xx_set_divider(struct inv_icm207xx * s, uint8_t idiv)
{
	int result = 0;
	...
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_SMPLRT_DIV, 1, &idiv);
	return result;
}

int inv_icm207xx_set_i2c_mst_dly(struct inv_icm207xx * s, uint16_t minDly, uint16_t minDly_compass)
{
	int result = 0;
	...
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_I2C_MST_DELAY_CTRL, 1, &delay_ctrl);
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_I2C_SLV4_CTRL, 1, &mst_delay);
 ...
 */
int inv_icm207xx_set_chip_power_state(struct inv_icm207xx * s, uint8_t func, uint8_t on_off)
{
	int status = 0;
	...
	case CHIP_AWAKE:
	{
		if(on_off) {
			if((s->base_state.wake_state & CHIP_AWAKE) == 0) {
				...
				inv_icm207xx_sleep_us(100); // after writting the bit wait 100 Micro Seconds
			}
		}
		else {
			if(s->base_state.wake_state & CHIP_AWAKE) {
				...
				status = serif_write(s, MPUREG_PWR_MGMT_1, 1, &s->base_state.pwr_mgmt_1);
				s->base_state.wake_state &= ~CHIP_AWAKE;
				inv_icm207xx_sleep_us(100); // after writing the bit wait 100 Micro Seconds
			}
		}
	}
	break;
...
uint8_t inv_icm207xx_get_chip_power_state(struct inv_icm207xx * s)
{
	return s->base_state.wake_state;
}

uint16_t inv_icm207xx_get_chip_base_sample_rate(struct inv_icm207xx * s)
{
	(void)s;
...
int inv_icm207xx_accel_fsr_2_reg(int32_t fsr)
{
	switch(fsr) {
...
int inv_icm207xx_reg_2_accel_fsr(uint8_t reg)
{
	switch(reg) {
...
int inv_icm207xx_set_accel_fullscale(struct inv_icm207xx * s, int level)
{
...
int inv_icm207xx_set_accel_ois_fullscale(struct inv_icm207xx * s, int level)
{
	if(level >= NUM_MPU_AFS)
		return INV_ERROR_BAD_ARG;
...
int inv_icm207xx_set_accel_bandwidth(struct inv_icm207xx * s, int level)
{
...
uint8_t inv_icm207xx_get_accel_fullscale(struct inv_icm207xx * s)
{
	return s->base_state.accel_fullscale;
}

uint8_t inv_icm207xx_get_accel_ois_fullscale(struct inv_icm207xx * s)
{
	return s->base_state.accel_ois_fullscale;
}

uint16_t inv_icm207xx_get_accel_bandwidth(struct inv_icm207xx * s)
{
	return s->base_state.accel_bw;
}

int inv_icm207xx_gyro_fsr_2_reg(int32_t fsr)
{
	switch(fsr) {
...
int inv_icm207xx_reg_2_gyro_fsr(uint8_t reg)
{
	switch(reg) {
...
int inv_icm207xx_set_gyro_fullscale(struct inv_icm207xx * s, int level)
{
...
int inv_icm207xx_set_gyro_ois_fullscale(struct inv_icm207xx * s, int level)
{
	if(level >= NUM_MPU_GFS)
		return INV_ERROR_BAD_ARG;
...
int inv_icm207xx_set_gyro_bandwidth(struct inv_icm207xx * s, int level)
{
...
uint8_t inv_icm207xx_get_gyro_fullscale(struct inv_icm207xx * s)
{
	return s->base_state.gyro_fullscale;
}

uint8_t inv_icm207xx_get_gyro_ois_fullscale(struct inv_icm207xx * s)
{
	return s->base_state.gyro_ois_fullscale;
}

uint16_t inv_icm207xx_get_gyro_bandwidth(struct inv_icm207xx * s)
{
	return s->base_state.gyro_bw;
}

int inv_icm207xx_is_advanced_features_supported(void)
{
#if (MEMS_CHIP == HW_ICM20690)
...
int inv_icm207xx_set_fsync_bit_location(struct inv_icm207xx * s, int bit_location)
{
	int result = 0;
#if (MEMS_CHIP == HW_ICM20690)
	...
	result |= inv_icm207xx_mems_write_reg(s, REG_CFG_USER_BANK_SEL, 1, &bank_sel);

	// enable the FSYNC ODR delay counter read by external interface - needed as this bit is set to 1 by default (HW related issue)
	result |= inv_icm207xx_mems_read_reg(s, REG_INTOSC_CTRL_NEW, 1, &config_reg);
	config_reg &= ~BIT_DELAY_TIME_RD_DISABLE;
	result |= inv_icm207xx_mems_write_reg(s, REG_INTOSC_CTRL_NEW, 1, &config_reg);

	// enable the FSYNC ODR delay counter configuration 
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_ACC_CTRL_NEW0, 1, &config_reg);
	config_reg |= BIT_ODR_DLY_TIME_EN_CFG;
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_ACC_CTRL_NEW0, 1, &config_reg);
	...
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_ANA_CTRL_NEW_1, 1, &config_reg);
	config_reg |= BIT_ODR_DELAY_TIME_EN;
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_ANA_CTRL_NEW_1, 1, &config_reg);

	// clear ext_sync_set field bits[5:3]
	result |= inv_icm207xx_mems_read_reg(s, MPUREG_CONFIG, 1, &config_reg);
	...
	result |= inv_icm207xx_mems_write_reg(s, MPUREG_CONFIG, 1, &config_reg);
#endif
